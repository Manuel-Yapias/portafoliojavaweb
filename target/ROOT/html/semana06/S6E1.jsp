...
<html lang="es">
            ...
            <details class="group bg-[#111] rounded-lg border border-[#222] transition-all duration-300 open:border-[var(--dorado)]">
                ...
                <div class="p-6 border-t border-[#222] space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Tarjeta 1: Modelo - procesarTransaccion -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>private void procesarTransaccion() {
    ...
    this.caramelosObsequio = calcularCaramelosObsequio(this.importePagar, this.cantidad);
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Método: procesarTransaccion()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Método orquestador con la <span class="text-[var(--dorado)]">única función</span> de secuenciar los cálculos de la venta delegando cada responsabilidad a sus submétodos especializados (precio, importe, descuento y caramelos de obsequio).
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 2: Modelo - Reglas de Negocio (Precios, Descuentos y Obsequios) -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>public double obtenerPorcentajeDescuento(int cant) {
    if (cant < 5) return 4.0;
    else if (cant < 10) return 6.5;
    else if (cant < 15) return 9.0;
    else return 11.5;
}
...
public int calcularCaramelosObsequio(double pagar, int cant) {
    ...
    return caramelosPorChocolate * cant;
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
                                <h5 class="text-white font-bold mb-2 text-sm">Cálculo de Tarifas y Bonificaciones</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Métodos con la <span class="text-[var(--dorado)]">única función</span> de evaluar las tablas tarifarias: determinan el porcentaje de descuento según el volumen de compra y la cantidad de caramelos otorgados (3 si el pago final es &ge; S/. 250, o 2 en caso contrario).
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-sky-400 overflow-x-auto border-l-2 border-sky-500">
                                <pre><code>int cantidad = Integer.parseInt(cantidadStr);

if (cantidad > 0) {
    S6E1Model liquidacion = 
        new S6E1Model(tipoChocolate, cantidad);
    ...
    request.setAttribute("resultado", 
        liquidacion);
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Validación e Inyección de Venta</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Encargado <span class="text-[var(--dorado)]">únicamente</span> de capturar los parámetros de entrada del formulario, validar la coherencia numérica de la cantidad e inyectar el modelo con la liquidación procesada en la petición HTTP.
                                </p>
                            </div>
                        </div>
