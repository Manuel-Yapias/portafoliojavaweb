...
<html lang="es">
            ...
            <details class="group bg-[#111] rounded-lg border border-[#222] transition-all duration-300 open:border-[var(--dorado)]">
                ...
                <div class="p-6 border-t border-[#222] space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Tarjeta 1: Modelo - obtenerPrecioUnitario -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>public double obtenerPrecioUnitario(String ruta, String calidad) {
    if ("huanuco".equals(r)) {
        switch (c) {
            case "A": return 45.0;
            case "B": return 35.0;
    ...
    return 0.0;
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Método: obtenerPrecioUnitario()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Tiene la <span class="text-[var(--dorado)]">única función</span> de cruzar la matriz bidimensional de destino (Huánuco/Huancayo) y calidad de bus (A, B, C) para devolver la tarifa unitaria por boleto.
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 2: Modelo - calcularPorcentajeDescuento -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>public double calcularPorcentajeDescuento(
        String calidad, int cant) {
    if ("A".equals(calidad) && cant > 4) {
    ...
    return 0.0;
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
                                <h5 class="text-white font-bold mb-2 text-sm">Método: calcularPorcentajeDescuento()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Método con la <span class="text-[var(--dorado)]">única función</span> de evaluar la regla condicional del negocio: otorga un 5% de descuento exclusivamente si se adquieren más de 4 pasajes en calidad 'A'.
                                </p>
                            </div>
                        </div>
...
if (cantidad > 0) {
    S6E2Model cotizacion = 
        new S6E2Model(ruta, calidad, cantidad);
    ...
    request.setAttribute("resultado", 
        cotizacion);
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Validación e Inyección de Cotización</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Encargado <span class="text-[var(--dorado)]">únicamente</span> de capturar la ruta, calidad y cantidad enviadas desde el formulario web, comprobar que los valores no sean nulos e instanciar la cotización en el modelo.
                                </p>
                            </div>
                        </div>
