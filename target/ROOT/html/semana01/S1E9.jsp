...
<head>
    ...
    <title>S1E9 - Reparto de Herencia</title>
...
<body class="min-h-screen flex flex-col p-4 md:p-6 justify-between">
    ...
    <header class="w-full max-w-[900px] mx-auto flex items-center justify-between py-5 px-6 bg-[#0d0d0d] border border-[#1c1c1c] rounded-t-lg">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-[10px] text-[var(--dorado)] tracking-widest font-mono uppercase">Semana 1 - Ejercicio 9</p>
            </div>
        </div>
    ...
    <main class="w-full max-w-[900px] mx-auto bg-[#0d0d0d] border-x border-[#1c1c1c] p-6 md:p-8 space-y-8">
        ...
        <div class="bg-[#111] p-4 rounded-lg border border-[#222]">
            ...
            <p class="text-sm text-gray-300 leading-relaxed">
                Un padre repartirá una cantidad de dinero entre sus cinco hijos de acuerdo con dependencias porcentuales específicas. Diseñe un programa que determine cuánto de dinero recibirá cada hijo.
            </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <form action="${pageContext.request.contextPath}/S1E9Controller" method="POST" class="space-y-4">
                <h3 class="text-sm font-mono text-white uppercase tracking-widest border-b border-[#222] pb-2">🎰 Repartición de Fondos</h3>
                
                <div>
                    <label for="monto" class="block text-xs font-mono text-gray-400 mb-1">Monto total a distribuir (S/.):</label>
            ...
            <div class="bg-[#121212] border border-[#222] rounded-lg p-6 flex flex-col justify-between">
                <div>
                    <h3 class="text-sm font-mono text-white uppercase tracking-widest border-b border-[#222] pb-2 mb-4">🏆 Libreta de Distribución</h3>
                    
                    <% if (request.getAttribute("error") != null) { %>
                        <div class="bg-red-950/40 border border-[var(--rojo-poker)] p-3 rounded text-xs text-red-300 font-mono">
                            ⚠ Error: <%= request.getAttribute("error") %>
                        </div>
                    <% } else if (request.getAttribute("josue") != null) { %>
                        <div class="space-y-3.5 font-mono text-xs">
                            <div class="flex justify-between items-center py-1 border-b border-zinc-900">
                                <span class="text-gray-400">Total a repartir:</span>
                                <span class="text-white font-bold">S/. ${String.format("%.2f", montoOriginal)}</span>
                            </div>
                            <div class="flex justify-between items-center py-1 border-b border-zinc-900">
                                <span class="text-[var(--dorado)]">Josué (27%):</span>
                                <span class="text-white font-bold">S/. ${String.format("%.2f", josue)}</span>
                            </div>
                            <div class="flex justify-between items-center py-1 border-b border-zinc-900">
                                <span class="text-[var(--dorado)]">Tamar (85% de Josué):</span>
                                <span class="text-white font-bold">S/. ${String.format("%.2f", tamar)}</span>
                            </div>
                            <div class="flex justify-between items-center py-1 border-b border-zinc-900">
                                <span class="text-[var(--dorado)]">Daniel (25%):</span>
                                <span class="text-white font-bold">S/. ${String.format("%.2f", daniel)}</span>
                            </div>
                            <div class="flex justify-between items-center py-1 border-b border-zinc-900">
                                <span class="text-[var(--dorado)]">Caleb (23% de J. y Dn.):</span>
                                <span class="text-white font-bold">S/. ${String.format("%.2f", caleb)}</span>
                            </div>
                            <div class="flex justify-between items-center py-1">
                                <span class="text-emerald-400">David (Sobrante):</span>
                                <span class="text-emerald-400 font-black text-sm">S/. ${String.format("%.2f", david)}</span>
                            </div>
                        </div>
                    <% } else { %>
                        <p class="text-xs text-gray-500 font-mono italic text-center py-20">Introduce la cifra total de dinero para calcular la división porcentual.</p>
                    <% } %>
                </div>
            </div>
        </div>
    ...
    request.setAttribute("josue", 
        modelo.calcularJosue()
    );
    request.setAttribute("tamar", 
        modelo.calcularTamar()
    );
    request.setAttribute("daniel", 
        modelo.calcularDaniel()
    );
    request.setAttribute("caleb", 
        modelo.calcularCaleb()
    );
    request.setAttribute("david", 
        modelo.calcularDavid()
    );
}</code></pre>
        </div>
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            ...
            <p class="text-gray-400 leading-relaxed">
                Encargado <span class="text-[var(--dorado)]">únicamente</span> de adjuntar al ámbito de petición (Request Scope) el monto original ingresado y los montos calculados individualmente para Josué, Tamar, Daniel, Caleb y David.
            </p>
        </div>
    </div>
    ...
    request.getRequestDispatcher(
        ...
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            ...
            <p class="text-gray-400 leading-relaxed">
                Tiene la <span class="text-[var(--dorado)]">función exclusiva</span> de delegar el flujo de ejecución reenviando la petición hacia la plantilla JSP del Ejercicio 9.
            </p>
        </div>
    </div>

    <!-- Método 3: calcularJosue() -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
        <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
            <pre><code>public double calcularJosue() {
    return this.montoTotal * 0.27;
        ...
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            ...
            <h5 class="text-white font-bold mb-2 text-sm">Método: calcularJosue()</h5>
            <p class="text-gray-400 leading-relaxed">
                Calcula la asignación para Josué equivalente al **27%** del monto total ingresado con <span class="text-[var(--dorado)]">responsabilidad única</span>.
            </p>
        </div>
    </div>

    <!-- Método 4: calcularTamar() -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
        <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
            <pre><code>public double calcularTamar() {
    return calcularJosue() * 0.85;
}</code></pre>
        </div>
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
            <h5 class="text-white font-bold mb-2 text-sm">Método: calcularTamar()</h5>
            <p class="text-gray-400 leading-relaxed">
                Tiene la <span class="text-[var(--dorado)]">única tarea</span> de calcular lo correspondiente a Tamar determinando el **85%** de la porción recibida por Josué.
            </p>
        </div>
    </div>

    <!-- Método 5: calcularDaniel() -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
        <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
            <pre><code>public double calcularDaniel() {
    return this.montoTotal * 0.25;
}</code></pre>
        </div>
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
            <h5 class="text-white font-bold mb-2 text-sm">Método: calcularDaniel()</h5>
            <p class="text-gray-400 leading-relaxed">
                Obtiene con <span class="text-[var(--dorado)]">responsabilidad exclusiva</span> la parte asignada a Daniel, equivalente al **25%** del monto total.
            </p>
        </div>
    </div>

    <!-- Método 6: calcularCaleb() -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
        <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
            <pre><code>public double calcularCaleb() {
    return (calcularJosue() + calcularDaniel()) 
            * 0.23;
}</code></pre>
        </div>
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
            <h5 class="text-white font-bold mb-2 text-sm">Método: calcularCaleb()</h5>
            <p class="text-gray-400 leading-relaxed">
                Su <span class="text-[var(--dorado)]">única tarea</span> es calcular el **23%** de la suma combinada de los montos obtenidos por Josué y Daniel.
            </p>
        </div>
    </div>

    <!-- Método 7: calcularDavid() -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
        <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
            <pre><code>public double calcularDavid() {
    double asignado = calcularJosue() 
            + calcularTamar() 
            + calcularDaniel() 
            + calcularCaleb();
    return this.montoTotal - asignado;
}</code></pre>
        </div>
        <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
            <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
            <h5 class="text-white font-bold mb-2 text-sm">Método: calcularDavid()</h5>
            <p class="text-gray-400 leading-relaxed">
                Determina con <span class="text-[var(--dorado)]">responsabilidad única</span> la cantidad remanente restando del fondo total la suma acumulada de las cuatro asignaciones anteriores.
            </p>
        </div>
    </div>
