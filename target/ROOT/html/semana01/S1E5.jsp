...
<head>
    ...
    <title>S1E5 - Área y Volumen de un Cubo</title>
...
<body class="min-h-screen flex flex-col p-4 md:p-6 justify-between">
    ...
    <header class="w-full max-w-[900px] mx-auto flex items-center justify-between py-5 px-6 bg-[#0d0d0d] border border-[#1c1c1c] rounded-t-lg">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-[10px] text-[var(--dorado)] tracking-widest font-mono uppercase">Semana 1 - Ejercicio 5</p>
            </div>
        </div>
    ...
    <main class="w-full max-w-[900px] mx-auto bg-[#0d0d0d] border-x border-[#1c1c1c] p-6 md:p-8 space-y-8">
        ...
        <div class="bg-[#111] p-4 rounded-lg border border-[#222]">
            ...
            <p class="text-sm text-gray-300 leading-relaxed">
                Diseñe un programa que determine el área (A) y el volumen (V) de un cubo del que se conoce su lado (l).
            </p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <form action="${pageContext.request.contextPath}/S1E5Controller" method="POST" class="space-y-4">
                <h3 class="text-sm font-mono text-white uppercase tracking-widest border-b border-[#222] pb-2">🎰 Ingreso de Datos</h3>
                
                <div>
                    <label for="lado" class="block text-xs font-mono text-gray-400 mb-1">Lado del Cubo (l):</label>
                    <input type="number" step="any" id="lado" name="lado" required 
                           value="${not empty lado ? lado : ''}"
                    ...
                    <% } else if (request.getAttribute("area") != null) { %>
                        <div class="space-y-4 font-mono">
                            <div class="flex justify-between items-center py-2 border-b border-[#1c1c1c]">
                                <span class="text-xs text-gray-400">Lado (l):</span>
                                <span class="text-sm text-white">${lado} u</span>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-[#1c1c1c]">
                            ...
                            <div class="flex justify-between items-center py-2">
                                ...
                                <span class="text-lg font-bold text-[var(--dorado)]">${volumen} u³</span>
                            </div>
                        </div>
                    <% } else { %>
                        <p class="text-xs text-gray-500 font-mono italic text-center py-16">Ingresa la dimensión del lado y haz clic en calcular.</p>
                    <% } %>
                </div>
            </div>
        </div>
    ...
    request.getRequestDispatcher(
        "/html/semana01/S1E1.jsp"
                    ...
                    <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                        ...
                        <p class="text-gray-400 leading-relaxed">
                            Su <span class="text-[var(--dorado)]">única función</span> es gestionar la redirección (forward) del flujo de ejecución hacia la interfaz del Ejercicio 1 para renderizar el HTML.
                        </p>
                    </div>
                </div>
    ...
    return this.base * this.altura;
                ...
                <!-- Método 5: calcularPerimetro() -->
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                    <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                        <pre><code>public double calcularPerimetro() {
    return 2 * (this.base + this.altura);
}</code></pre>
                    </div>
                    <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                        <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
                        <h5 class="text-white font-bold mb-2 text-sm">Método: calcularPerimetro()</h5>
                        <p class="text-gray-400 leading-relaxed">
                            Su <span class="text-[var(--dorado)]">única responsabilidad</span> es calcular el contorno o perímetro total del rectángulo sumando sus lados y multiplicando el resultado por dos.
                        </p>
                    </div>
                </div>
