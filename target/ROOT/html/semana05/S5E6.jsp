...
<html lang="es">
            ...
            <details class="group bg-[#111] rounded-lg border border-[#222] transition-all duration-300 open:border-[var(--dorado)]">
                ...
                <div class="p-6 border-t border-[#222] space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Tarjeta 1: Modelo - generarYAnalizarInventario -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>private void generarYAnalizarInventario() {
    Random random = new Random();
    for (int i = 1; i <= 100; i++) {
        int stock = random.nextInt(201);
        if (stock < 50) this.catA++;
        else if (stock < 100) this.catB++;
        else if (stock < 150) this.catC++;
        else this.catD++;
        
        if (stock > stockMaximo) stockMaximo = stock;
        if (stock < stockMinimo) stockMinimo = stock;
    }
}</code></pre>
                            </div>
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Método: generarYAnalizarInventario()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Tiene la <span class="text-[var(--dorado)]">única función</span> de simular pseudoaleatoriamente el stock para 100 productos (0 a 200), clasificarlos en las 4 categorías por rango, y extraer los límites máximos, mínimos y el promedio general.
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 2: Modelo - Clase Anidada Producto -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>public static class Producto {
    ...
    public int getStock() { return stock; }
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // DTO</span>
                                <h5 class="text-white font-bold mb-2 text-sm">Estructura Interna: Producto</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Encargada <span class="text-[var(--dorado)]">únicamente</span> de empaquetar los datos individuales de cada ítem (código formateado como <code class="text-[var(--dorado)]">PROD-001</code> y cantidad de stock) para su lectura directa desde la vista.
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 3: Controlador - Generación al Disparo (doPost) -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
    ...
    request.setAttribute("resultado", 
        crupierInventario);
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Disparo y Generación de Lote</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Encargado <span class="text-[var(--dorado)]">únicamente</span> de responder a la petición del usuario instanciando una nueva ejecución del modelo (nuevo lote de 100 productos) y reexpedir el resultado a la vista JSP.
                                </p>
                            </div>
                        </div>
