...
<html lang="es">
            ...
            <details class="group bg-[#111] rounded-lg border border-[#222] transition-all duration-300 open:border-[var(--dorado)]">
                ...
                <div class="p-6 border-t border-[#222] space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Tarjeta 1: Modelo - procesarMensaje -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>private String procesarMensaje() {
    ...
    for (String palabra : PALABRAS_PROHIBIDAS) {
        ...
        while (indice != -1) {
            ...
            indice = temporalMinusculas.indexOf(palabra, 
    ...
    return temporal;
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Método: procesarMensaje()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Tiene la <span class="text-[var(--dorado)]">única función</span> de evaluar la cadena de texto, contabilizar las apariciones del diccionario de términos no permitidos y reemplazar cada ocurrencia de forma insensible a mayúsculas por asteriscos (<code class="text-[var(--dorado)]">***</code>).
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 2: Modelo - fueMensajeModificado -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-[var(--dorado)] overflow-x-auto border-l-2 border-[var(--dorado)]">
                                <pre><code>public boolean fueMensajeModificado() {
    return this.palabrasCensuradas > 0;
}</code></pre>
                            </div>
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                <span class="text-[10px] text-[var(--dorado)] tracking-wider mb-1 uppercase font-bold">🧮 MODELO // NEGOCIO</span>
                                <h5 class="text-white font-bold mb-2 text-sm">Método: fueMensajeModificado()</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Método de consulta con la <span class="text-[var(--dorado)]">única función</span> de verificar si el mensaje ingresado sufrió alteraciones durante el proceso de filtrado.
                                </p>
                            </div>
                        </div>

                        <!-- Tarjeta 3: Controlador - doPost (Procesamiento del Chat) -->
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 bg-[#141414] p-4 rounded border border-zinc-800 hover:border-zinc-700 transition-all">
                            <div class="lg:col-span-5 bg-black p-3 rounded font-mono text-[10px] text-sky-400 overflow-x-auto border-l-2 border-sky-500">
                                <pre><code>String chatInput = request.getParameter("chatInput");

if (chatInput != null && !chatInput.trim().isEmpty()) {
    S7E2Model chatFiltro = 
        new S7E2Model(chatInput);
        
    request.setAttribute("chatFiltro", 
        chatFiltro);
    request.setAttribute("exito", true);
                            ...
                            <div class="lg:col-span-7 flex flex-col justify-center text-xs font-mono">
                                ...
                                <h5 class="text-white font-bold mb-2 text-sm">Captura e Inyección de Moderación</h5>
                                <p class="text-gray-400 leading-relaxed">
                                    Encargado <span class="text-[var(--dorado)]">únicamente</span> de receptar el mensaje enviado por el usuario, validar que no sea una cadena vacía, invocar al filtro de moderación y transmitir la respuesta procesada a la vista JSP.
                                </p>
                            </div>
                        </div>
