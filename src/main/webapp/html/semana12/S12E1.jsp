...
<body class="min-h-screen flex flex-col justify-between p-4 md:p-8">
    ...
    <script>
        // Objeto JSON con el manual de uso super detallado para cada paso técnico real (Excluyendo Fase 1, 2 y 3).
        const pasosData = {
            1: {
        ...
        function activarPaso(numero) {
            ...
            for (let i = 1; i <= 7; i++) {
                document.getElementById(`btn-step-${i}`).classList.remove('step-active');
            }
            
            // Añadir estilo activo al presionado
            document.getElementById(`btn-step-${numero}`).classList.add('step-active');
